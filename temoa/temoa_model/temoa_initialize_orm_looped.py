...
def CreateSparseDicts(M: 'TemoaModel'):
    ...
    for r, i, t, v, o in M.Efficiency.sparse_iterkeys():
        ...
        for p in M.time_optimize:
            ...
            if (t in M.tech_reserve or t in M.tech_curtailment) and (r, p, t) not in M.curtailmentVintages: # using curtailment as an operating reserve
            ...
            if (t in M.tech_reserve or t in M.tech_curtailment): # using curtailment as an operating reserve
...
# For prototype operating reserve margin
def MaxSeasonalActivityReserveIndices(M: 'TemoaModel'):
    indices = set(
        (r, p, s, d, t)
        for r, p, s, t in M.MaxSeasonalActivity.keys()
        for d in M.time_of_day
    )
    return indices
...
# For prototype operating reserve margin
def StorageReserveIndices(M: 'TemoaModel'):
    indices = set(
        (r, p, s, d, t, v)
        for r, p, s, d, t, v in M.StorageConstraints_rpsdtv
        if t in M.tech_reserve
    )
    return indices
